...
class BelendBot extends BootBot{
    ...
    constructor(options){
        ...
        /**
         * Store the dir of actions
         * @type {String} || {null}
         */
        this.actionDir = options.witActionDir;
        ...
        if (options.witToken) {
            if(this.actionDir){
                const that = this;
                //Load default send method for wit.ai
                let actions = {
                    send({sessionId}, {text}) {
                        const recipientId = that.sessions[sessionId].fbid;
                        if (recipientId) {
                            that.sendTextMessage(recipientId, text, null, {typing: true});
                        } else {
                ...
                //Load more actions from actionDir
                const actionPath = path.join(this.actionDir, '');
                require("fs").readdirSync(actionPath).forEach(function(file) {
                    const loadedActions = require(path.join(actionPath, file))(that);
                    actions = Object.assign(loadedActions, actions);
                });

                this.witActions = actions;
            }
